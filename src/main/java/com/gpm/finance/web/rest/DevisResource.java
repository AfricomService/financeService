...
/**
 * REST controller for managing {@link com.gpm.finance.domain.Devis}.
 */
@RestController
@RequestMapping("/api")
public class DevisResource {
    ...
    private static final String ENTITY_NAME = "financeServiceDevis";
    ...
    public ResponseEntity<List<DevisDTO>> getAllDevis(
        ...
        Page<DevisDTO> page;
        if (eagerload) {
            page = devisService.findAllWithEagerRelationships(pageable);
        } else {
            page = devisService.findAll(pageable);
        }
